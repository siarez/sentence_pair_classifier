...
y = tf.layers.dense(inputs=v1_v2_1, units=2, activation=tf.nn.sigmoid, use_bias=True,
                kernel_initializer=None, bias_initializer=None,
                     name='H')
...
with tf.Session() as sess:
    ...
    label_feed = np.array([1])

    for i in range(1000):
        y1, losss, y2, one_hots = sess.run([train_op, loss, y, onehot_labels], {a: a_feed, b: b_feed, label: label_feed})
        if i % 100 == 0:
            print(losss, y2, one_hots)
